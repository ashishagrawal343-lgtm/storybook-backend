...
function buildCoverPrompt(themeInput, archetypeInput, child, options = {}) {
    ...
    // 3. Child Hero Spotlight Directive (Guarantees Fit-to-Window & Zero Cropping of Feet/Head)
    const heroSpotlightPart = `${childAnchor} as ${childRole}, the central heroic focal point occupying 60 to 65 percent of the canvas, beautifully framed and fitted to the dedicated lower hero window: ${heroAction}; full-length figure with head, hair, body, legs, and feet comfortably contained within the dedicated hero zone with natural breathing margins so no feet, shoes, or limbs are cut off, and the child appears naturally proportioned; wide soulful sparkling eyes, an authentic radiant smile of delight and wonder, natural dimensional skin tones with gentle warmth, crisp expressive features, and heroic posture; the child's head and hair remain strictly below the 35 percent horizontal line with zero encroachment into the upper title zone;`;
