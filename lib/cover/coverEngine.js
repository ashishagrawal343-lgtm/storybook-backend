        ...
        if (characterReferenceUrl) {
            ...
            try {
                ...
                const headwearDirective = rawHeadwear
                    ? `[CRITICAL CULTURAL ACCURACY: The child is wearing an authentic ${rawHeadwear.replace(/^authentic\s+/i, '')}; faithfully preserve this exact headwear.] `
                    : `[STRICT HEADWEAR RESTRICTION: The child has natural hair with NO headwear, NO hat, NO cap. Strictly preserve their natural hair.] `;
                const glassesDirective = (attributes && attributes.hasGlasses)
                    ? `[CRITICAL VISUAL FEATURE: The child is wearing ${attributes.glassesDescription || 'spectacles'}; faithfully preserve the spectacles on their face.] `
                    : `[STRICT EYEWEAR RESTRICTION: The child does NOT wear glasses. Strictly preserve their natural face with zero eyewear.] `;
                const propsDirective = `[STRICT NO-PROPS RULE: Maintain the child's natural appearance as-is from the photo without adding extra props or costume accessories.] `;

                const forbiddenItems = ['extra handheld props', 'unneeded costume accessories'];
                if (!attributes || !attributes.hasHeadwear) forbiddenItems.push('hats', 'caps', 'head coverings');
                if (!attributes || !attributes.hasGlasses) forbiddenItems.push('spectacles', 'glasses', 'sunglasses');
                ...
                // Directly generate avatar using flux-kontext-pro with attribute-aware prompt
                const avatarUrl = await genFn(avatarPrompt, photoData, { aspect_ratio: '1:1', isFace: true, upscale: false });
                if (avatarUrl) {
                    characterReferenceUrl = String(avatarUrl);
                    characterReferenceAssetId = `ref_${crypto.createHash('sha256').update(characterReferenceUrl).digest('hex').slice(0, 12)}`;
                    console.log(`✅ [CoverEngine v2] Premium painterly character anchor locked (glasses preserved: ${!!(attributes && attributes.hasGlasses)}): ${characterReferenceUrl}`);
                }
            } catch (avatarErr) {
