...
const THEME_MASTERS = {
    ...
    'forest-and-animals': {
        ...
        heroAction: 'sitting on a mossy root gently feeding or whispering to a friendly fawn and rabbit, beaming with joy.',
    ...
    'kingdom-and-princess': {
        ...
        allowedMotifs: ['soft heirloom roses', 'distant castle spires', 'golden starlight motes', 'golden ribbons', 'friendly doves'],
    ...
    'dinosaur-wonders': {
        ...
        visualNorthStar: 'A lush, sunlit prehistoric paradise of giant prehistoric ferns, glowing waterfalls, and gentle, playful young dinosaurs.',
        ...
        heroAction: 'gently petting a playful little triceratops or laughing as a friendly young long-neck reaches down to nuzzle their shoulder.',
        heroFocalArea: 'commands 60% to 65% of the canvas together with their friendly little dinosaur companion.',
        lighting: 'Warm amber prehistoric sunlight pouring through misty giant fern leaves, golden rim lighting.',
        texture: 'Rich gouache and oil painting, soft organic textures, velvety foliage.',
        ...
        allowedMotifs: ['friendly gentle herbivores', 'giant emerald ferns', 'fossil amber gems', 'gentle waterfalls', 'tropical blooms'],
    ...
    'ocean-and-dolphins': {
        ...
        heroAction: 'standing joyfully beside a gentle glowing dolphin in a crystal marine sanctuary, smiling with gentle wonder.',
        heroFocalArea: 'occupies 60% to 65% of the composition in harmonious friendship with the dolphin.',
        lighting: 'Soft aquatic shimmer and iridescent bioluminescent glow from reef corals.',
    ...
    'bedtime-lullabies': {
        ...
        childRole: 'A cozy little dreamer in soft pastel star bedtime pajamas, holding a favorite soft plushie.',
    ...
    'safari-and-jungle': {
        ...
        visualNorthStar: 'A vibrant African golden savannah and emerald rainforest border where friendly young elephants and giraffes roam under acacia trees.',
        ...
        childRole: 'An intrepid little wildlife explorer in comfortable explorer attire, khaki trousers, and a small brass magnifying glass.',
        heroAction: 'standing happily beside a playful young elephant holding a wildflower with its trunk, gazing out across the golden plains.',
        ...
        texture: 'Rich painterly impasto textures, warm grassland brushwork, gentle elephant textures and explorer fabrics.',
        ...
        allowedMotifs: ['young gentle elephants', 'friendly giraffes', 'golden acacia trees', 'sunflower sunbeams', 'colorful birds'],
...
/**
 * Sanitizes childRole string to prevent theme costumes from overriding authentic headwear.
 * Replaces generic caps, conductor caps, top hats, flower crowns with the child's authentic headwear.
 */
function sanitizeChildRoleForHeadwear(childRole, attributes = {}) {
    if (!childRole || typeof childRole !== 'string') return childRole || '';

    const hasHeadwear = attributes && attributes.hasHeadwear;
    ...
    let sanitized = childRole;

    if (hasHeadwear && rawHeadwear) {
        // Child authentically wears cultural headwear in their photo: preserve it faithfully
        const headwearName = rawHeadwear.startsWith('authentic ') ? rawHeadwear : `authentic ${rawHeadwear}`;
        sanitized = sanitized
            .replace(/in an adorable conductor cap,?\s*/gi, `wearing an ${headwearName}, `)
            .replace(/and a joyful top hat/gi, `and wearing an ${headwearName}`)
            .replace(/and a petal-woven flower crown/gi, `and wearing an ${headwearName}`)
            .replace(/with a petal-woven flower crown/gi, `wearing an ${headwearName}`)
            .replace(/a petal-woven flower crown/gi, `an ${headwearName}`)
            .replace(/\b(conductor cap|top hat|baseball cap|beanie|helmet|flower crown|party hat|straw hat|fedora|beret|visor|tiara|crown)\b/gi, headwearName);
    } else {
        // Child has natural hair in photo: strictly STRIP all hats, caps, crowns, tiaras, and headwear completely!
        sanitized = sanitized
            .replace(/in an adorable conductor cap,?\s*(?=brass-buttoned)/gi, 'in an elegant ')
            .replace(/in an adorable conductor cap,?\s*/gi, '')
            .replace(/and a joyful top hat/gi, '')
            .replace(/and a petal-woven flower crown/gi, '')
            .replace(/with a petal-woven flower crown/gi, '')
            .replace(/a petal-woven flower crown/gi, '')
            .replace(/\b(conductor cap|top hat|baseball cap|beanie|helmet|flower crown|party hat|straw hat|fedora|beret|visor|tiara|crown|hat|cap)\b/gi, '');
    }

    // Always strip unrequested handheld props, spectacles, or extra accessories
    sanitized = sanitized
        .replace(/and pocket watch/gi, '')
        .replace(/with a tiny satchel/gi, '')
        .replace(/and a small brass magnifying glass/gi, '')
        .replace(/\b(glasses|spectacles|goggles|monocle)\b/gi, '');
