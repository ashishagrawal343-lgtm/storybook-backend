...
function sanitizeChildRoleForHeadwear(childRole, attributes = {}) {
    ...
    if (!attributes || !attributes.hasHeadwear) return childRole;

    const headwearName = attributes.headwearDescription || (attributes.headwearType !== 'none' ? `authentic ${attributes.headwearType}` : 'authentic turban');

    let sanitized = childRole
        // Specific theme replacements
        ...
        // Generic headwear replacements
        .replace(/\b(conductor cap|top hat|baseball cap|beanie|helmet|flower crown|party hat|straw hat|fedora|beret|visor)\b/gi, headwearName);
