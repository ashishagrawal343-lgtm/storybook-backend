...
function calculateCoverZones(canvasWidth = 600, canvasHeight = 800) {
    ...
    const heroTop = titleTop + titleHeight + 20;            // ~240px @ 800 (minimum 20px gap after title)
    const heroHeight = Math.round(H * 0.625);               // ~500px @ 800 (Y: 240 to 740, exactly 62.5% height)
    const heroWidth = W - (sideMargin * 2);                 // ~528px @ 600 (within safe side margins)

    const footerTop = heroTop + heroHeight + 4;             // ~744px @ 800
    const footerHeight = H - footerTop - bottomMargin;      // ~28px @ 800
    const footerWidth = W - (sideMargin * 2);

    const totalCanvasArea = W * H;
    const heroBandArea = W * heroHeight;                    // 600 * 500 = 300,000 (62.5% spotlight band)
    const heroAreaRatio = Math.round((heroBandArea / totalCanvasArea) * 100) / 100;
